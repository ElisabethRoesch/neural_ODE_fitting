...
for key_t in key_list
    ...
    a = plot(ode_data[test[1],:], ode_data[test[2],:],
        label = "", ylim = (-1.7,1.7), xlim = (-1.7,1.7) ,xticks= ([-1,1],["",""]), yticks=  ([-1,1],["",""]), size=(500,500), margin=5Plots.mm,
        xlab = species[test[1]],linewidth=3, ylab = species[test[2]], grid = "off", framestyle = :box,
